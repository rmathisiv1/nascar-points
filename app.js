...
const STATE = {
  ...
  //   chart   — career-trajectory chart settings: series + metric + display.
  //             display "per-season" plots the metric per calendar year;
  //             "cumulative" plots a running total against career year
  //             (each driver's first FT season = Year 1). Cumulative
...
// so re-hovering re-opens normally. Subitems also force-blur themselves
// so :focus-within doesn't latch the menu open either.
function wireTabDropdowns() {
  document.querySelectorAll(".dash-tab-group").forEach(grp => {
    // Click on parent tab OR any subitem → mark group, force-blur active
    // element, allow nav to proceed
    grp.querySelectorAll(".dash-tab-parent, .dash-subitem").forEach(a => {
      a.addEventListener("click", () => {
        grp.classList.add("just-clicked");
        // Defer blur so the click navigation actually fires first
        setTimeout(() => {
          if (document.activeElement && grp.contains(document.activeElement)) {
...
// update the toggle so it's clear what state the site is in. Series
// switcher, R-cursor, and search all stay active in both modes.
...
function wireUIControls() {
  ...
  // Resets to the most recent (season, race), clears any time-cursor
  // scroll-back, snaps to NCS, and routes to the default standings
  // landing. Replaces the old "Now" button (which did the same thing).
  document.getElementById("brand-link")?.addEventListener("click", async (e) => {
    e.preventDefault();
    const targetSeason = (STATE.seasonsAvailable && STATE.seasonsAvailable[0])
      || STATE.season;
    const targetSeries = "NCS";   // Always returns to the top series
...
const OWNER_TO_TEAM_CODE = {
  ...
  // === Historical Cup primary teams (pre-2014) ===
  "Chip Ganassi":                  "CGR",
  ...
  "Doug Bawel":                    "JEG",
  "Beth Ann Morgenthau":           "TBR",
  "Dietrich Mateschitz":           "TBR",
  "Red Bull Racing Team":          "TBR",
  "James Finch":                   "PHR",
  "Phoenix Racing":                "PHR",
  "Tommy Baldwin, Jr.":            "TBR2",
  ...
  "Mike Curb":                     "HAT",
  "Shigeaki Hattori":              "HAT",
  "Hattori Racing Enterprises":    "HAT",
  "Charlie Henderson":             "CHR",
  "Mark Beaver":                   "BVM",
  "Jennifer Jo Cobb":              "JJC",
  "Jennifer Jo Cobb Racing":       "JJC",
  "Robert Richardson, Sr.":        "RRR",
  "George Debidart":               "DEB",

  // === Existing Xfinity-only ===
  "Jeremy Clements":          "JCR",
  "Jimmy Means":              "JMR",
  "Jordan Anderson":          "JAR",
  "Mike Harmon":              "MHR",
  ...
  // === Misc additional historical ===
  "Kevin Buckler":            "TRG",
  "Derrike Cope":             "CPE",
  "Andy Belmont":             "BMT",
  "Kirk Shelmerdine":         "KSR",
  "Morgan Shepherd":          "MSR2",
  ...
  "Jeff Wyler":               "WYL",
  "Archie St. Hilaire":       "GO",
  "Go Green Racing":          "GO",
  "Go FAS Racing":            "GO",
  "Jerry Brown":              "JBR2",
  "Bill Lewis":               "LWS",

  // === Second-pass historical additions ===
  "Gregg Mixon":              "MIX",
  "MSRP Motorsports":         "MSRP",
  "Randy Moss":               "RMM",
  ...
  "Dusty Whitney":            "DWM",
  "John McGill":              "MMS",
  "Bill Baumgardner":         "BMS",
  "Brian Baumgardner":        "BMS",
  "Ed Evans":                 "EVA",
  "Joe Falk":                 "CIR",
  "Circle Sport":             "CIR",
  "Jason Sciavicco":          "JSR",
  "Chance2 Motorsports":      "CH2",
  "John McNelly":             "MNR",
  "Dave Malcolmson":          "DMR",
  "StarCom Racing":           "STR",
  "Joe Reilly":               "JRY",
  "Bryan Mullet":             "BMR2",
  "Emerling-Gase Motorsports":"EGM",
  "Steve Coulter":            "CLR",
  "Stanley Herzog":           "HZG",
  "Steven Lane":              "LNR",
  "Sam Rensi":                "SRR",
...
// Expand a single team card to fill the Teammate grid; collapse back to grid
// view when the user dismisses. The other cards aren't removed — just hidden —
// so any in-flight observers keep working and re-rendering returns instantly.
function openTeammateZoom(teamKey, srcCard) {
  if (!srcCard) return;
  const host = document.getElementById("teammates-grid");
  if (!host) return;

  // Mark this card as the zoomed one; hide all sibling cards.
  host.classList.add("tm-grid-zoomed");
  host.querySelectorAll(".tm-card").forEach(c => {
    if (c === srcCard) c.classList.add("tm-card-zoomed");
    else c.classList.add("tm-card-hidden");
  });

  // Inject a back-button bar if not already present
  let backBar = host.querySelector(".tm-zoom-back");
  if (!backBar) {
    backBar = document.createElement("button");
    backBar.className = "tm-zoom-back";
    backBar.type = "button";
    backBar.innerHTML = "← Back to all teams";
    host.insertBefore(backBar, host.firstChild);
    backBar.addEventListener("click", closeTeammateZoom);
  }

  // ESC also closes
  if (!host._zoomKeyHandler) {
    host._zoomKeyHandler = (e) => { if (e.key === "Escape") closeTeammateZoom(); };
    document.addEventListener("keydown", host._zoomKeyHandler);
  }

  // Repaint sparklines at the new wider container
  requestAnimationFrame(() => tmPaintSparklines(host));
}

function closeTeammateZoom() {
  const host = document.getElementById("teammates-grid");
  if (!host) return;
  host.classList.remove("tm-grid-zoomed");
  host.querySelectorAll(".tm-card").forEach(c => {
    c.classList.remove("tm-card-zoomed", "tm-card-hidden");
  });
  const back = host.querySelector(".tm-zoom-back");
  if (back) back.remove();
  if (host._zoomKeyHandler) {
    document.removeEventListener("keydown", host._zoomKeyHandler);
    host._zoomKeyHandler = null;
  }
  // Repaint sparklines back at grid widths
  requestAnimationFrame(() => tmPaintSparklines(host));
}
...
function tmPaintSparklines(root) {
  ...
  svgs.forEach(svg => {
    ...
    // the default 42 — e.g. the zoom modal uses 60).
    const H = Math.max(20, Math.floor(rect.height));
    // Extra padding so circle DOTS at the extremes don't clip against the
    // SVG edge. Open rings at finish=1 sit at y=padY with stroke radius 3.7;
    // we want ≥ that much clearance.
    const padY = Math.max(7, Math.round(H * 0.22));
...
function renderProfile() {
  ...
  // Shows each championship's series tag + year count broken out so the
  // user can see at a glance "2 NCS · 1 NTS" rather than just a total.
  ...
  const champTooltip = champData.byYear
    .map(c => `${c.year} ${c.series}`)
    .join(" · ");
  // Bucket by series so we can render per-series mini pills
  const champBySeries = { NCS: 0, NOS: 0, NTS: 0 };
  champData.byYear.forEach(c => { champBySeries[c.series] = (champBySeries[c.series] || 0) + 1; });
  const seriesPillsHTML = ["NCS", "NOS", "NTS"]
    .filter(s => champBySeries[s] > 0)
    .map(s => `<span class="champ-series-pill series-${s.toLowerCase()}">${champBySeries[s]} ${s}</span>`)
    .join("");
  const champLine = champData.total > 0
    ? `<span class="profile-hero-champ" title="${escapeHTML(champTooltip)}">
         ...
         <span class="champ-series-breakdown">${seriesPillsHTML}</span>
       </span>`
    : "";
...
function computeDriverLoopAggregates(driverName) {
  ...
  ["NCS", "NOS", "NTS"].forEach(code => {
    ...
    // record so the new track-type panel can read multiple metrics
    // off the same record without a second walk.
    const ratings = [];
    Object.keys(SEASON_CACHE).forEach(yearStr => {
      const year = parseInt(yearStr, 10);
      ...
      block.races.forEach(r => {
        ...
        ratings.push({
          ...
          date: r.date,
          track_code: r.track_code,
          // Optional secondary loop metrics — may be null on older races.
...
//   { super: { count, avg_rating, avg_pass_diff, avg_top15_pct, avg_finish, races: [...] }, ... }
// Keys are present only if the driver has >=1 rated race of that type.
// `seriesScope` is an array like ["NCS"] (present mode) or all three
// series codes (historical mode). Matches the rating-chart convention.
function computeDriverTrackTypeLoop(driverName, seriesScope) {
  ...
  const m = arr => arr.length ? arr.reduce((s, x) => s + x, 0) / arr.length : null;
  const out = {};
  Object.keys(buckets).forEach(t => {
    const b = buckets[t];
    out[t] = {
      count:          b.ratings.length,
      avg_rating:     m(b.ratings),
      avg_pass_diff:  m(b.pass_diffs),
      avg_top15_pct:  m(b.top15s),
      avg_finish:     m(b.finishes),
      races:          b.races,
    };
  });
  return out;
...
// Hides itself if fewer than 2 track types have data (single bar is
// not "by category" — it's just one number, already shown elsewhere).
function paintProfileTrackTypeLoop(driverName) {
  ...
  // Canonical track-type display order so the panel reads the same
  // across drivers (super, short, inter, road).
  ...
  // Color thresholds. Driver Rating: same green/red anchors used in
  // the Loop Stats tab (>=100 hot, <70 cold). Pass diff: positive is
  // good. %Top15: high is good. Avg finish: low is good.
...
function renderCompareCareerChart(drivers) {
  ...
  // each year's metric value plotted on a calendar-year X axis;
  // Cumulative shows running totals against career year (year 1, 2, 3
  // since the driver's first FT season in the series). Cumulative is
  // the one to use for "how fast did A reach B's career total" reading.
  ...
  // Controls header — emitted regardless of whether there's data so the
  // user can change settings even when the current pick is empty.
  const controlsHTML = `
    <div class="cmp-chart-controls">
      ...
      <span class="cmp-chart-control-label">Display</span>
      <div class="toggle-group mini" id="cmp-chart-display-toggle">${displayToggleHTML}</div>
    </div>
  `;
  ...
  // ----- Build per-driver point series based on display mode -----
  // Each driver gets {points: [{x, y, val, year, careerYear, starts}], label}.
  // In per-season mode, x = calendar year, y = metric value.
  // In cumulative mode, x = career year (1-based), y = running total.
  const driverSeries = driverData.map(d => {
    if (displayMode === "cumulative") {
      // Sort ascending and accumulate. careerYear is the 1-based index
      // of each full-time season, so all drivers start at x=1.
      let running = 0;
      const points = d.yearly.map((y, i) => {
        running += y[metricChoice] || 0;
        return {
          x: i + 1,              // career year (1-based)
          y: running,            // cumulative total
          val: running,          // shown in tooltip
          year: y.year,          // calendar year (for tooltip)
          careerYear: i + 1,
          starts: y.starts,
        };
      });
      return { driver: d.driver, points };
    } else {
      // Per-season: x = calendar year, y = that season's value
  ...
  // Determine axis ranges across all series
  const allXVals = driverSeries.flatMap(s => s.points.map(p => p.x));
  const allYVals = driverSeries.flatMap(s => s.points.map(p => p.y));
  const minX = displayMode === "cumulative" ? 1 : Math.min(...allXVals);
  const maxX = Math.max(...allXVals);
  const maxY = Math.max(...allYVals, 1); // floor at 1
  ...
  const xFor = (x) => M.l + ((x - minX) / (maxX - minX || 1)) * plotW;
  const yFor = (val) => M.t + plotH - (val / maxY) * plotH;

  // X ticks
  const xSpan = maxX - minX + 1;
  let xTickStep = 1;
  if (xSpan > 16) xTickStep = 5;
  else if (xSpan > 8) xTickStep = 2;
  const xTicks = [];
  for (let x = minX; x <= maxX; x += xTickStep) xTicks.push(x);
  if (xTicks[xTicks.length - 1] !== maxX) xTicks.push(maxX);

  // Y ticks — nice round step sizes
  const niceStep = (max) => {
    const target = max / 4;
  ...
  if (yTicks[yTicks.length - 1] < maxY) yTicks.push(yTicks[yTicks.length - 1] + yStep);

  // X tick label formatter — career year gets a "Yr N" prefix on the
  // first and last; intermediate ticks just show the number.
  const xTickFormat = (x, i, arr) => {
    if (displayMode === "cumulative") {
      // First or last tick gets the prefix to disambiguate from calendar years
      if (i === 0 || i === arr.length - 1) return `Yr ${x}`;
      return String(x);
    }
    return String(x);
  };
  ...
  const xTickLabels = xTicks.map((x, i, arr) =>
    `<text class="cmp-chart-tick" x="${xFor(x)}" y="${H - 14}" text-anchor="middle">${xTickFormat(x, i, arr)}</text>`
  ).join("");
  ...
  // One line per driver
  const linesHTML = driverSeries.map((s, di) => {
    ...
    const pathD = points.map((p, i) =>
      `${i === 0 ? "M" : "L"}${p.svgX.toFixed(1)},${p.svgY.toFixed(1)}`
    ).join(" ");

    // Tooltip text depends on display mode.
    //   per-season: "Tyler Reddick · 2023 · 3 wins in 36 starts"
    //   cumulative: "Tyler Reddick · 2023 (Yr 4) · 12 wins through this season"
    const dotsHTML = points.map(p => {
      const tooltipText = (displayMode === "cumulative")
        ? `${s.driver.name} · ${p.year} (Yr ${p.careerYear}) · ${p.val} career ${metricLower} through this season`
        : `${s.driver.name} · ${p.year} · ${p.val} ${metricLower} in ${p.starts} starts`;
      return `<circle class="cmp-chart-dot"
         cx="${p.svgX.toFixed(1)}" cy="${p.svgY.toFixed(1)}" r="3.5"
  ...
  return `
    <div class="cmp-section">
      <div class="cmp-section-head">
        <span class="cmp-section-title">${sectionTitle}</span>
      </div>
      ${controlsHTML}
...
function wireCompareEventHandlers() {
  ...
  const displayToggle = document.getElementById("cmp-chart-display-toggle");
  if (displayToggle && !displayToggle._wired) {
    displayToggle._wired = true;
    displayToggle.addEventListener("click", (e) => {
      const btn = e.target.closest("button[data-cmp-chart-display]");
      if (!btn) return;
...
function renderTeamPage() {
  ...
  // Championship count (across all series). Hero gold-star pill with a
  // per-series breakdown so the user can see how many championships came
  // from each series at a glance — important for teams like JGR (Cup +
  // Xfinity) or Hendrick (mostly Cup).
  const teamChamps = countTeamChampionships(teamCode);
  const champTip = teamChamps.byYear
    .map(c => `${c.year} ${c.series} · ${c.driver}`)
    .join("\n");
  const champBySeries = { NCS: 0, NOS: 0, NTS: 0 };
  teamChamps.byYear.forEach(c => { champBySeries[c.series] = (champBySeries[c.series] || 0) + 1; });
  const teamSeriesPillsHTML = ["NCS", "NOS", "NTS"]
    .filter(s => champBySeries[s] > 0)
    .map(s => `<span class="champ-series-pill series-${s.toLowerCase()}">${champBySeries[s]} ${s}</span>`)
    .join("");
  const champLine = teamChamps.total > 0
    ? `<div class="tm-hero-champ" title="${escapeHTML(champTip)}">
         ...
         <span class="champ-series-breakdown">${teamSeriesPillsHTML}</span>
       </div>`
    : "";

  // Multi-series team stats. Roll up wins/T5/T10/poles/lapsLed for THIS
  // team across every series for the current year, so a single block
  // surfaces "what is this team doing right now everywhere they race".
  // The original STATE.series row is preserved as the highlighted card.
  const teamStatsBySeries = computeTeamStatsBySeries(teamCode);
...
// Roll up team stats per series for the current season. Returns an object
...
function computeTeamStatsBySeries(teamCode) {
  ...
  ["NCS", "NOS", "NTS"].forEach(sCode => {
    const block = blocks[sCode];
    if (!block || !block.races) {
      out[sCode] = { wins: 0, top5: 0, top10: 0, poles: 0, lapsLed: 0, cars: 0 };
      return;
    }
    const seriesEntities = entitiesFromRaces(block.races);
    const teamEntities = seriesEntities.filter(e => {
    ...
    let wins = 0, top5 = 0, top10 = 0, poles = 0, lapsLed = 0;
    ...
    out[sCode] = { wins, top5, top10, poles, lapsLed, cars: teamEntities.length };
  });
  return out;
}
...
function computeTeamHistory(teamCode) {
  const rows = [];
  // Show every (year, series) combo this team has fielded a car. The
  // historical table is the team's complete record — filtering it to one
  // series hides too much (e.g., JGR's NOS championships disappear when
  // the user is in NCS). Series tag in each row keeps them distinguishable.
...
function renderTeamHistoryTable(rows, teamCode) {
  ...
  const body = yearKeys.map((year, yearIdx) => {
    ...
    return yrRows.map((r, rowIdx) => {
      ...
      // Color the rank pill the same way we color cells in the heatmap so a
      // championship year (P1) reads gold, top-5 reads green, etc.
      const rankPillClass = (rank) => {
        if (rank == null) return "";
        if (rank === 1) return " ph-win";
        if (rank <= 5) return " ph-t5";
        if (rank <= 10) return " ph-t10";
        if (rank <= 20) return " ph-mid";
        if (rank <= 30) return " ph-down";
        return " ph-bot";
      };
      const rankCell = r.standingsRank != null
        ? `<td class="num"><span class="rank-pill${rankPillClass(r.standingsRank)}">P${r.standingsRank}</span></td>`
        : `<td class="num muted">—</td>`;
      // Wins cell — gold pill if there are any. Gold scales with magnitude
      // (1-2 wins muted, 3+ wins bright).
      const winsCellClass = r.wins === 0
        ? "num muted"
        : "num";
      const winsContent = r.wins === 0
        ? "0"
        : `<span class="wins-pill${r.wins >= 3 ? " wins-many" : ""}">${r.wins}</span>`;
      ...
      return `<tr class="${blockClass}${rowIdx === 0 ? " yr-first" : ""}">
        ...
        <td class="${winsCellClass}">${winsContent}</td>
        ${rankCell}
...
//
// Memoized — walking 100k+ result rows on every keystroke (sort flip,
// search debounce trigger) would freeze the UI. We cache by the set of
// loaded years so a freshly-loaded season invalidates correctly.
// ============================================================

const ALLTIME_CACHE = { drivers: null, teams: null, crewchiefs: null, sig: null };
function allTimeCacheSig() {
  // Stable signature reflecting which years are loaded. Whenever this
  // changes we know to recompute. Sorted to keep the string deterministic.
  return Object.keys(SEASON_CACHE).sort().join(",");
}
function invalidateAllTimeIfStale() {
  const sig = allTimeCacheSig();
  if (ALLTIME_CACHE.sig !== sig) {
    ALLTIME_CACHE.sig = sig;
    ALLTIME_CACHE.drivers = null;
    ALLTIME_CACHE.teams = null;
    ALLTIME_CACHE.crewchiefs = null;
  }
}
...
function computeAllTimeDrivers() {
  ...
  return Array.from(byDrv.values());
}
...
// to drivers/teams/CCs whose careers fell within those eras.
const NASCAR_GENERATIONS = [
  { id: "g1", label: "Gen 1", year_start: 1948, year_end: 1966 },
  { id: "g2", label: "Gen 2", year_start: 1967, year_end: 1980 },
  { id: "g3", label: "Gen 3", year_start: 1981, year_end: 1990 },
  { id: "g4", label: "Gen 4", year_start: 1991, year_end: 2006 },
  { id: "g5", label: "Gen 5", year_start: 2007, year_end: 2012 },
  { id: "g6", label: "Gen 6", year_start: 2013, year_end: 2021 },
...
//   pageLabel  — "Drivers" | "Teams" | "Crew Chiefs" (singular/plural label)
//
// Folds each record by the active scope + gen filters, then sorts +
// paginates. Pagination defaults to 50 per page so render stays fast
// even with ~3000 historical drivers.
function renderAllTimeTable(rawRecs, linkBuilder, stateKey, pageLabel) {
  ...
  const body = `<tbody>${pageRows.map((r, i) => {
    const href = linkBuilder ? linkBuilder(r) : null;
    const nameCell = href
      ? `<a class="profile-link" href="${href}">${escapeHTML(r.name)}</a>`
      : escapeHTML(r.name);
    const overallRank = startIdx + i + 1;
    return `<tr>
      <td class="num alltime-rank-cell">${overallRank}</td>
      <td>${nameCell}</td>
...
// Wire toolbar controls (search input + click-to-sort headers + scope
// toggle + generation chips + pagination) for an all-time page. Re-runs
// the appropriate render function on input/click. Search input is
// debounced 150ms so each keystroke doesn't rebuild the entire table.
...
// while a load is in flight no-op. The done callback ONLY fires when
// at least one season was loaded; if everything was already cached we
// return synchronously without invoking it (otherwise the caller's
// rerender → ensureAllSeasonsCached → rerender pattern recurses
// infinitely with no work in between to break it).
const ALLSEASONS_LOAD = { inflight: false, generation: 0 };
async function ensureAllSeasonsCached(done) {
  const missing = (STATE.seasonsAvailable || []).filter(y => !SEASON_CACHE[y]);
  if (missing.length === 0) return;   // already done, no callback needed
      ...
      <table class="data-table po-table">
        ...
        <tbody>
          ${firstOut.map((r, i) => {
            ...
            const txt = contrastTextFor(carHex);
            const teamPill = renderTeamPill(r.team_code);
            // Rank continues seamlessly from the last chase seed.
            // Field is seeds 1..N, bubble is N+1, N+2, ... The chase
            // format only cares about eligible drivers, so we never
            // need to surface ineligible ones in this view — they're
            // simply not in the championship hunt.
            const rank = rule.field + i + 1;
            const back = cutoffPts - r.total;
            const linkHref = poView === "driver"
              ? `#/driver/${encodeURIComponent(r.key)}`
              : `#/car/${r.car_number}`;
