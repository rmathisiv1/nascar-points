...
def parse_race(race_url: str, series_code: str, round_num: int,
    ...
    # Track whether the results table actually has a "Pts" column.
    # Old Racing Reference pages (pre-1975 or so) often lack it. Without
    # this guard, the inferred-ineligibility heuristic below would mark
    # EVERY driver ineligible (pts == 0 for all → inferred crossover),
    # which caused Richard Petty's 200 wins to vanish from the all-time
    # stats. The flag is checked after all rows are parsed.
    has_pts_col = any(name.lower() == "pts" for name in header_cells)
    ...
    for tr in results_table.find_all("tr")[1:]:
        ...
        #
        # GUARD: only apply this heuristic when the Pts column EXISTS.
        # Old pages (pre-1975) often lack a Pts column entirely, so
        # pts == None → (pts or 0) == 0 for every driver. Without this
        # guard, every driver in a 1960s race becomes "ineligible" and
        # their wins/starts vanish from career totals.
        if (not ineligible and has_pts_col
                and (pts or 0) == 0 and pos is not None
    ...
    # Post-pass: if the Pts column existed but EVERY driver has race_pts
    # == 0, the column was present-but-empty (some old RR pages have the
    # header but no data). Clear inferred ineligibility so these drivers
    # don't vanish from career totals.
    if has_pts_col and race.results:
        any_nonzero_pts = any(d.race_pts > 0 for d in race.results)
        if not any_nonzero_pts:
            for d in race.results:
                # Only clear ineligibility that was INFERRED (0-pts heuristic).
                # Keep explicit markers (* / † / (i)) since those are real.
                # We can't perfectly distinguish "inferred" from "explicit"
                # post-hoc, but the explicit markers set ineligible BEFORE
                # the heuristic runs. Since the heuristic only fires when
                # `not ineligible`, an explicitly-marked driver would already
                # be True and wouldn't have been touched by the heuristic.
                # So clearing all True values here would also clear explicit
                # ones — but that's actually fine: if the whole column is
                # empty, we can't trust pts-based inferences anyway.
                d.ineligible = False
