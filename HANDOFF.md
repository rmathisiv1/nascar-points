...
Moved the panel UP to sit right after the Driver Rating panel (with Performance Profile etc.), out of its old spot below the Career Heatmap. Series toggle now has an **All** option (`getDriverTopTracks` accepts series="all" → aggregates history + pace across NCS/NOS/NTS). Rows now EXPAND INLINE on click (showing laps-led/race, top-15% rate, pace vs leader, wins·top5, plus a "View track page" link) instead of navigating away — handler keys on `[data-tt-row]`, toggles the sibling `[data-tt-detail]`. (Still active-driver profile branch only.)
