...
- ~~Owner / manufacturer projection views.~~ DONE. Owner view mirrors the driver view but is CAR-CENTRIC: baseline from owner totals (`pointsMapThroughRound`, keyed `#<car>`) + the same projected race gain. Labels are car numbers only — the colored car pill is the identifier (no doubled "#45" text): `_renderProjectionTopContenders` and `_renderProjectionChaseTable` take a `carOnly` flag (pill only, no name) used by the owner view; the owner reg-season bar chart labels by `#<car>` (set in `_ownerizeProj`); owner standings table dropped its Driver column (pill + team). Owner pts == driver pts for full-time eligible single-driver cars, so the front of the field matches the driver view by design. Mfr view = champ-picture cards + projected-points bar chart + standings table. Cards rank by champ% (title odds, derived from the same chase traces as the driver cards so they reconcile). The chart + table use REAL NASCAR manufacturer points — `manufacturerStandingsThroughRound` / `mfrPositionPoints` (best finisher of each make per race, 2014+ scale), the SAME computation as the Standings → Manufacturer page, so current totals match it exactly. Projected = current × (all races / completed) (manufacturer title runs every race, no reset); chart solid = current, light = projected; chart + table both sort by projected mfr pts. Earlier metrics (summed points, champ% bar, avg-per-car) were all rejected — avg-per-car notably did NOT match the Standings page, which is what prompted the real-scale switch.

PART-TIME DRIVERS EXCLUDED FROM PROJECTIONS (all series). The entity filter is car-centric, so a full-time CAR with a part-time representative driver (e.g. Corey Heim running a handful of NTS races while full-time in Cup) was being projected as a full-season contender and even projected champion. `simulateSeasonRollout` now also requires the DRIVER's personal eligible-start count (`driverStartCounts`, ineligible/crossover starts excluded) to clear the same `projMinRaces = completed − 3` bar. PROJ_VERSION bumped 22→23 to invalidate cached projections. Note: the projection field is full-time-only, so a driver's projection rank can differ from the Standings page rank (Standings lists everyone incl. part-timers) — that's expected, not a bug.
