...
- ~~Owner / manufacturer projection views.~~ DONE. Owner view mirrors the driver view but is CAR-CENTRIC: baseline points come from owner totals (`pointsMapThroughRound`, keyed `#<car>`) + the same projected race gain, and every label is the car number (no driver names — `_ownerizeProj` sets `name = #<car>`; owner contenders/chase get the same relabel inline in `_buildProjectionHTML`; owner standings table dropped its Driver column). Note: owner pts == driver pts for full-time eligible single-driver cars, so the front of the field reads identically to the driver view by design; they diverge only for cars that ran a relief/ineligible driver. Mfr view = championship-picture cards + standings table only (NO bar chart): champ% is derived from the same chase traces as the driver cards so the two reconcile; cards + table both sort by champ%. The mfr bar chart was removed — no points ordering matches the champ%-sorted cards/table (summed points rewards car count; a champ% bar just duplicates the cards), so it was dropped rather than left contradictory.
